...
import { UserPlus, TrendingUp, TrendingDown, Minus, ExternalLink, Activity, Users, Shield, Zap } from "lucide-react";
...
function RegisterModal({onClose}:{onClose:()=>void}) {
  const [wallet,setWallet]=useState(""); const [name,setName]=useState("");
  const [tier,setTier]=useState(3); const [loading,setLoading]=useState(false); const [done,setDone]=useState(false);
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,backdropFilter:"blur(4px)"}}>
      <div style={{background:"var(--bg-1)",border:"1px solid var(--border-2)",padding:"40px",width:"100%",maxWidth:"480px",position:"relative"}}>
        <button onClick={onClose} style={{position:"absolute",top:"16px",right:"16px",background:"none",border:"none",color:"var(--text-3)",cursor:"pointer",fontSize:"20px",lineHeight:1}}>?</button>
        {!done ? (
          <>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
              <div><label style={{...m,fontSize:"9px",color:"var(--text-3)",letterSpacing:"0.1em",textTransform:"uppercase",display:"block",marginBottom:"6px"}}>Issuer Wallet (Solana Pubkey)</label><input value={wallet} onChange={e=>setWallet(e.target.value)} placeholder="e.g. EDBYT2E8HGEKhTRmHH..." style={inp}/></div>
          ...
          <div style={{textAlign:"center",padding:"20px 0"}}>
            <div style={{width:"48px",height:"48px",background:"var(--accent-bg)",border:"1px solid var(--accent-border)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}}>
            ...
            <div style={{display:"flex",gap:"12px",justifyContent:"center"}}>
              <a href="/issue" style={{...m,fontSize:"10px",color:"var(--accent)",letterSpacing:"0.08em",textTransform:"uppercase"}}>Issue Attestation ?</a>
  ...
  return (
    <AdminShell current="/protocol">
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"28px"}}>
        ...
        <button onClick={()=>setShowRegister(true)} style={{display:"flex",alignItems:"center",gap:"8px",...m,fontSize:"11px",letterSpacing:"0.1em",textTransform:"uppercase",background:"var(--accent)",color:"white",border:"none",padding:"10px 20px",cursor:"pointer",fontWeight:600}}>
        ...
        <div style={{border:"1px solid var(--border)",borderTop:"none",background:"var(--bg-1)"}}>
          ...
          {ISSUERS.map((iss,i)=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 140px 60px 100px 90px 100px",gap:"12px",padding:"15px 20px",borderBottom:i<ISSUERS.length-1?"1px solid var(--border)":"none",alignItems:"center",borderLeft:`2px solid ${iss.status==="active"?"var(--accent)":"var(--danger)"}`,background:"var(--bg-1)"}}>
              ...
              <span style={{...m,fontSize:"9px",padding:"3px 8px",background:iss.status==="active"?"rgba(22,163,74,0.08)":"var(--danger-bg)",color:iss.status==="active"?"var(--success)":"var(--danger)",border:`1px solid ${iss.status==="active"?"rgba(22,163,74,0.2)":"var(--danger-border)"}`,display:"inline-block",letterSpacing:"0.06em",textTransform:"uppercase"}}>
                {iss.status}
              </span>
              <span style={{...m,fontSize:"10px",color:"var(--text-3)"}}>{iss.registered}</span>
            </div>
          ))}
          <div style={{padding:"12px 20px",borderTop:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center",background:"var(--bg-1)"}}>
            <span style={{...m,fontSize:"9px",color:"var(--text-4)"}}>{ISSUERS.length} registered ? {ISSUERS.filter(x=>x.status==="active").length} active</span>
            <button onClick={()=>setShowRegister(true)} style={{display:"flex",alignItems:"center",gap:"6px",...m,fontSize:"9px",letterSpacing:"0.08em",textTransform:"uppercase",background:"transparent",color:"var(--accent)",border:"1px solid var(--accent-border)",padding:"6px 14px",cursor:"pointer"}}>
              <UserPlus size={11}/> Register New
            </button>
          </div>
        </div>
      )}
      ...
      {tab==="activity" && (
        <div style={{border:"1px solid var(--border)",borderTop:"none",background:"var(--bg-1)"}}>
          ...
          {RECENT_TX.map((tx,i)=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"160px 1fr 1fr 80px",gap:"12px",padding:"13px 20px",borderBottom:i<RECENT_TX.length-1?"1px solid var(--border)":"none",alignItems:"center",borderLeft:`2px solid ${TX_META[tx.type].color}`,background:"var(--bg-1)"}}>
