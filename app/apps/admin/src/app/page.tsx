...
import { ShieldCheck, Vault, BadgeCheck, BookOpen, ScrollText, Radio, Download, LayoutDashboard, ArrowRight, Lock } from "lucide-react";

const m = {fontFamily:"'DM Mono',monospace"};
const f = {fontFamily:"'Inter',sans-serif"};

const SCREENS = [
  {icon:ShieldCheck, title:"Protocol Overview",    role:"Super Admin",       href:"/protocol", roles:["super_admin"]},
...
const ROLE_DESCS: Record<string,string> = {
  super_admin:        "Full protocol authority. Register operators and issuers, monitor all vaults.",
  ...
  none:               "This wallet is not registered in the Leyfis protocol. Contact your Super Admin.",
};
  ...
  if(!publicKey) return (
    <div style={{minHeight:"100vh",display:"grid",gridTemplateColumns:"1fr 1fr",background:"var(--bg)"}}>
      <div style={{position:"relative",padding:"48px",display:"flex",flexDirection:"column",justifyContent:"space-between",borderRight:"1px solid var(--border)",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:"-80px",left:"-80px",width:"360px",height:"360px",background:"radial-gradient(circle,rgba(27,79,216,0.06) 0%,transparent 70%)",pointerEvents:"none"}}/>
        ...
        <div style={{position:"relative"}}>
          <div style={{...m,fontSize:"9px",color:"var(--accent)",letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:"16px",display:"flex",alignItems:"center",gap:"8px"}}>
            <span style={{display:"block",width:"16px",height:"1px",background:"var(--accent)"}}/>Compliance infrastructure
          </div>
        ...
        <div style={{position:"relative",display:"flex",flexDirection:"column",gap:"10px"}}>
          {[{icon:ShieldCheck,t:"Protocol-level enforcement",d:"7-check validation on every interaction"},{icon:BadgeCheck,t:"Role-gated access",d:"Wallet-based auth with on-chain detection"},{icon:ScrollText,t:"Immutable audit trail",d:"Every gate call recorded permanently"},{icon:Download,t:"FATF R.16 compliant",d:"One-click CSV export for regulators"}].map((f2,i)=>{
            const Icon=f2.icon;
            return (
              <div key={i} style={{display:"flex",alignItems:"flex-start",gap:"12px",padding:"12px 14px",border:"1px solid var(--border)",background:"var(--bg-1)"}}>
                ...
                <div><div style={{...f,fontSize:"12px",fontWeight:600,color:"var(--text-1)",marginBottom:"2px"}}>{f2.t}</div><div style={{...m,fontSize:"9px",color:"var(--text-3)"}}>{f2.d}</div></div>
              </div>
            );
          })}
        </div>
        ...
        <div style={{width:"100%",maxWidth:"360px"}}>
          <div style={{border:"1px solid var(--border)",background:"var(--bg-1)",padding:"36px",marginBottom:"20px"}}>
            ...
            <div style={{height:"1px",background:"var(--border)",marginBottom:"24px",position:"relative"}}>
              <span style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",...m,fontSize:"9px",color:"var(--text-4)",background:"var(--bg-1)",padding:"0 10px",letterSpacing:"0.1em",textTransform:"uppercase",whiteSpace:"nowrap"}}>Connect Wallet</span>
            </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
              ...
              {[["Super Admin","Full protocol control"],["Vault Operator","Configure vaults"],["KYC Issuer","Issue credentials"],["Compliance Auditor","Read-only audit"]].map(([r,d])=>(
        ...
        {SCREENS.map(s=>{
          ...
          return (
            <a key={s.href} href={accessible?s.href:"#"}
              style={{border:`1px solid ${accessible?"var(--border)":"var(--border)"}`,padding:"20px 24px",display:"flex",flexDirection:"column",gap:"10px",textDecoration:"none",opacity:accessible?1:0.35,cursor:accessible?"pointer":"not-allowed",background:"var(--bg-1)",transition:"border-color 0.15s,background 0.15s"}}
