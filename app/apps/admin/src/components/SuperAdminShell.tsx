  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px" }}>
      ...
      <div style={{ position:"relative", width:"100%", maxWidth:"440px" }}>
        ...
        <div style={{ width:"56px", height:"56px", background:"rgba(196,68,68,0.08)", border:"1px solid rgba(196,68,68,0.24)", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:"24px" }}>
          <Lock size={22} color="var(--danger)"/>
        </div>
        <div style={{ ...m, fontSize:"9px", color:"var(--danger)", letterSpacing:"0.16em", textTransform:"uppercase", marginBottom:"12px" }}>403 — Access Restricted</div>
        ...
        <div style={{ border:"1px solid var(--border)", background:"var(--bg-1)", padding:"16px 20px", marginBottom:"24px" }}>
          <div style={{ ...m, fontSize:"9px", color:"var(--text-4)", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"10px" }}>Authentication Check</div>
          <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
            {[["Route","/superadmin"],["Required","VaultConfig.authority"],["Gate Program",shortAddr(GATE_PROGRAM_ID,8)],["Status",reason==="no_wallet"?"No wallet":"Not authorized"]].map(([l,v],i)=>(
              <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", ...(i===3?{paddingTop:"8px",borderTop:"1px solid var(--border)"}:{}) }}>
                <span style={{ ...m, fontSize:"10px", color:"var(--text-3)" }}>{l}</span>
                <span style={{ ...m, fontSize:"10px", color:i===3?"var(--danger)":"var(--text-2)", fontWeight:i===3?600:400, letterSpacing:i===3?"0.06em":"0", textTransform:i===3?"uppercase":"none" }}>{v}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginTop:"40px", paddingTop:"20px", borderTop:"1px solid var(--border)" }}>
          <p style={{ ...m, fontSize:"9px", color:"var(--text-4)", lineHeight:1.8, textAlign:"center" }}>Access attempts are logged on-chain.<br/>Leyfis Protocol — Solana devnet</p>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"16px" }}>
        ...
        <span style={{ fontFamily:"DM Mono,monospace", fontSize:"10px", color:"var(--text-4)", letterSpacing:"0.12em", textTransform:"uppercase" }}>Verifying authority on-chain...</span>
      </div>
      <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    </div>
  );
    ...
    <div style={{ display:"flex", minHeight:"100vh", background:"var(--bg)" }}>
      <aside style={{ width:"var(--sidebar-w)", flexShrink:0, borderRight:"1px solid var(--border)", display:"flex", flexDirection:"column", position:"fixed", top:0, left:0, bottom:0, zIndex:300, background:"var(--bg-1)" }}>
        <div style={{ padding:"24px 20px 20px", borderBottom:"1px solid var(--border)" }}>
          <a href="/superadmin" style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"12px" }}>
            ...
            <div>
              ...
              <div style={{ fontFamily:"DM Mono,monospace", fontSize:"9px", color:"var(--text-4)", letterSpacing:"0.1em", textTransform:"uppercase", marginTop:"3px" }}>Protocol Console</div>
            </div>
          </a>
          <div style={{ display:"flex", alignItems:"center", gap:"6px", padding:"6px 10px", background:"var(--accent-bg)", border:"1px solid var(--accent-border)" }}>
            <ShieldCheck size={11} color="var(--accent)"/>
            <span style={{ fontFamily:"DM Mono,monospace", fontSize:"9px", color:"var(--accent)", letterSpacing:"0.1em", textTransform:"uppercase", fontWeight:600 }}>Super Admin</span>
          </div>
        </div>
        <nav style={{ flex:1, padding:"10px 0", overflowY:"auto" }}>
          {SA_NAV.map(item => {
            ...
            return (
              <a key={item.href} href={item.href} style={{ display:"flex", alignItems:"center", gap:"12px", padding:"11px 20px", textDecoration:"none", background:active?"var(--accent-bg)":"transparent", borderLeft:active?"2px solid var(--accent)":"2px solid transparent", transition:"background 0.15s" }}>
          ...
          <div style={{ margin:"12px 20px", height:"1px", background:"var(--border)" }}/>
          <a href="/" style={{ display:"flex", alignItems:"center", gap:"10px", padding:"10px 20px", textDecoration:"none", opacity:0.5 }}>
            <ExternalLink size={14} color="var(--text-4)"/>
            <span style={{ fontFamily:"DM Mono,monospace", fontSize:"10px", color:"var(--text-4)", letterSpacing:"0.06em" }}>Institutional Console</span>
          </a>
        </nav>
        <div style={{ padding:"16px 20px", borderTop:"1px solid var(--border)" }}>
          ...
          {publicKey && (
            <div style={{ background:"var(--bg-2)", border:"1px solid var(--accent-border)", padding:"12px" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"6px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
                  <Wallet size={11} color="var(--accent)"/>
                  <span style={{ fontFamily:"DM Mono,monospace", fontSize:"9px", color:"var(--accent)", letterSpacing:"0.08em", textTransform:"uppercase" }}>Authority</span>
                </div>
              ...
              <div style={{ fontFamily:"DM Mono,monospace", fontSize:"10px", color:"var(--text-2)", fontWeight:500 }}>{shortAddr(publicKey.toBase58(),8)}</div>
            </div>
          )}
        </div>
      </aside>
      ...
      <main style={{ flex:1, marginLeft:"var(--sidebar-w)", minHeight:"100vh", background:"var(--bg)" }}>
        <div style={{ position:"sticky", top:0, zIndex:200, padding:"14px 36px", display:"flex", alignItems:"center", justifyContent:"space-between", background:"var(--bg-1)", borderBottom:"1px solid var(--border)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
            {(()=>{ const item=SA_NAV.find(n=>n.href===current); if(!item) return null; const Icon=item.icon; return <><Icon size={15} color="var(--text-3)"/><span style={{ fontFamily:"DM Mono,monospace", fontSize:"11px", color:"var(--text-3)", letterSpacing:"0.12em", textTransform:"uppercase" }}>{item.label}</span></>; })()}
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"6px", padding:"4px 10px", background:"var(--accent-bg)", border:"1px solid var(--accent-border)" }}>
              <ShieldCheck size={11} color="var(--accent)"/>
              <span style={{ fontFamily:"DM Mono,monospace", fontSize:"9px", color:"var(--accent)", letterSpacing:"0.08em", textTransform:"uppercase" }}>Protocol Authority</span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
