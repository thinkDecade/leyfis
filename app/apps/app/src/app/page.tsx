...
// ─── Slide image panel ────────────────────────────────────────
function ImagePanel({ src, position = "center", overlay = true }: {
  src: string; position?: string; overlay?: boolean;
}) {
  return (
    <div style={{ position:"relative", background:D.bgDark, overflow:"hidden" }}>
      ...
      {overlay && (
        <div style={{ position:"absolute", inset:0, background:"linear-gradient(135deg,rgba(10,15,28,0.35) 0%,rgba(10,15,28,0.10) 50%,rgba(10,15,28,0.28) 100%)" }}/>
      )}
    </div>
  );
}
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .slide-grid{display:grid;grid-template-columns:1fr 1fr;min-height:560px}
        .slide-grid-rev{display:grid;grid-template-columns:1fr 1fr;min-height:560px}
        @media(max-width:900px){.slide-grid,.slide-grid-rev{grid-template-columns:1fr}}
      `}</style>
      ...
      <section style={{ minHeight:"100vh",display:"flex",flexDirection:"column",justifyContent:"flex-end",position:"relative",overflow:"hidden",background:D.bgDark }}>
        ...
        <div style={{ position:"relative",zIndex:1,padding:"140px 8vw 72px" }}>
          <div style={{ animation:"fadeUp .55s ease both",marginBottom:0 }}>
      ...
      {/* ── §01 THE GAP — slide deck: text left | padlock image right ── */}
      <section id="problem" style={{ borderTop:`1px solid ${D.rule}` }}>
        <div className="slide-grid">

          {/* TEXT COLUMN */}
          <div style={{ padding:"88px 7vw 88px 8vw", background:D.bg1, display:"flex", flexDirection:"column", justifyContent:"center" }}>
            <Fade>
              ...
              <h2 style={{ fontFamily:D.display,fontSize:"clamp(26px,3.2vw,48px)",fontWeight:700,lineHeight:1.03,letterSpacing:"-.028em",color:D.ink,marginBottom:24 }}>
                Compliance exists.<br/><span style={{ color:D.indigo }}>DeFi can't read it.</span>
              </h2>
              <p style={{ fontFamily:D.sans,fontSize:"16px",color:D.ink50,lineHeight:1.75,marginBottom:16 }}>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:3,marginBottom:32 }}>
                <div style={{ padding:"20px 24px",background:D.red10,border:"1px solid rgba(122,31,31,0.16)" }}>
                ...
                <div style={{ padding:"20px 24px",background:D.teal10,border:`1px solid ${D.teal20}` }}>
              ...
              {/* Agentic callout */}
              <div style={{ padding:"22px 26px",background:D.indigo12,border:`1px solid ${D.indigo20}`,display:"flex",gap:20,alignItems:"flex-start" }}>
                <div style={{ flexShrink:0,width:40,height:40,background:D.indigo20,display:"flex",alignItems:"center",justifyContent:"center" }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={D.indigo} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:D.mono,fontSize:"10px",letterSpacing:".10em",color:D.indigo,marginBottom:8,textTransform:"uppercase" }}>New · Leyfis for the Agentic Economy</div>
                  <h3 style={{ fontFamily:D.display,fontSize:"15px",fontWeight:700,color:D.ink,marginBottom:8 }}>Your AI agents inherit your compliance tier.</h3>
                  <p style={{ fontFamily:D.sans,fontSize:"13px",color:D.ink50,lineHeight:1.7 }}>In the agentic economy, institutions deploy AI systems to execute on their behalf. Leyfis verifies them exactly as it verifies your human traders.</p>
                </div>
              </div>
            </Fade>
          </div>

          {/* IMAGE COLUMN — padlock / network: compliance locked from DeFi */}
          <Fade delay={80} style={{ display:"contents" }}>
            <div style={{ position:"relative", background:D.bgDark, overflow:"hidden", minHeight:480 }}>
              ...
              {/* Edge gradients for blending */}
              <div style={{ position:"absolute",inset:0,background:"linear-gradient(to right,rgba(10,15,28,0.55) 0%,transparent 30%,transparent 70%,rgba(10,15,28,0.55) 100%),linear-gradient(to bottom,rgba(10,15,28,0.40) 0%,transparent 20%,transparent 80%,rgba(10,15,28,0.40) 100%)" }}/>
              {/* Caption */}
              <div style={{ position:"absolute",bottom:36,left:36,right:36 }}>
                <div style={{ fontFamily:D.mono,fontSize:"10px",letterSpacing:".12em",color:"rgba(232,238,246,0.35)",textTransform:"uppercase" }}>Compliance Infrastructure</div>
                <div style={{ fontFamily:D.sans,fontSize:"13px",fontWeight:500,color:"rgba(232,238,246,0.55)",marginTop:6 }}>On-chain credentials. Permissioned access.</div>
              </div>
            </div>
          </Fade>
        </div>
      ...
      {/* ── §02 THE GATE — slide deck: gate image left | text + checks right ── */}
      <section id="how" style={{ borderTop:`1px solid ${D.rule}` }}>
        <div className="slide-grid-rev">

          {/* IMAGE COLUMN — left — document + checkmark: attestation */}
          <Fade delay={60} style={{ display:"contents" }}>
            <div style={{ position:"relative", background:D.bgDark, overflow:"hidden", minHeight:520, order:0 }}>
              ...
              <div style={{ position:"absolute",inset:0,background:"linear-gradient(to right,rgba(10,15,28,0.40) 0%,transparent 30%,transparent 70%,rgba(10,15,28,0.40) 100%),linear-gradient(to bottom,rgba(10,15,28,0.35) 0%,transparent 20%,transparent 80%,rgba(10,15,28,0.35) 100%)" }}/>
              {/* Floating label */}
              <div style={{ position:"absolute",top:40,left:40 }}>
                <div style={{ display:"inline-flex",alignItems:"center",gap:8,background:"rgba(27,79,216,0.22)",border:"1px solid rgba(27,79,216,0.38)",padding:"5px 14px" }}>
                  ...
                  <span style={{ fontFamily:D.mono,fontSize:"10px",color:"rgba(100,140,255,0.9)",letterSpacing:".10em" }}>SEVEN CHECKS · EVERY CALL</span>
                </div>
              </div>
              {/* Gate flow overlay */}
              <div style={{ position:"absolute",bottom:40,left:36,right:36 }}>
                {[
                  { from:"Wallet →",    label:"Leyfis Gate",  color:"rgba(27,79,216,0.9)"  },
                  { from:"Gate →",      label:"7 Checks",     color:"rgba(180,83,9,0.9)"   },
                  { from:"PASS →",      label:"Vault (CPI)",  color:"rgba(15,110,86,0.9)"  },
                  { from:"FAIL →",      label:"Rejected",     color:"rgba(122,31,31,0.9)"  },
                ].map(({ from,label,color },i) => (
                  <div key={i} style={{ display:"flex",alignItems:"center",gap:12,padding:"9px 16px",marginBottom:2,background:"rgba(10,15,28,0.70)",border:"1px solid rgba(232,238,246,0.07)",backdropFilter:"blur(6px)" }}>
                    <span style={{ fontFamily:D.mono,fontSize:"11px",color:"rgba(232,238,246,0.35)",minWidth:60 }}>{from}</span>
                    <span style={{ fontFamily:D.mono,fontSize:"12px",color,fontWeight:500 }}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* TEXT COLUMN — right */}
          <div style={{ padding:"88px 8vw 88px 7vw", background:D.bg2, display:"flex", flexDirection:"column", justifyContent:"center", order:1 }}>
            <Fade>
              <div style={{ fontFamily:D.mono,fontSize:"11px",letterSpacing:".14em",color:D.ink30,textTransform:"uppercase",marginBottom:40 }}>02 — The Gate</div>
              <h2 style={{ fontFamily:D.display,fontSize:"clamp(26px,3.2vw,48px)",fontWeight:700,lineHeight:1.03,letterSpacing:"-.028em",color:D.ink,marginBottom:24 }}>
                Seven checks.<br/>Every time.<br/><span style={{ color:D.indigo }}>No exceptions.</span>
              </h2>
              <p style={{ fontFamily:D.sans,fontSize:"16px",color:D.ink50,lineHeight:1.75,marginBottom:40 }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:3,marginBottom:36 }}>
                {[
                  ...
                  { n:"02", t:"Gate",      d:"Every vault interaction hits the Gate first. Seven checks fire in order. No discretion." },
                  { n:"03", t:"Authorize", d:"All checks pass. Leyfis CPIs to the vault. Atomic. Invisible to compliant users." },
                  { n:"04", t:"Audit",     d:"Every outcome writes an immutable AuditEntry on-chain. FATF R.16 CSV on demand." },
                ].map(({ n,t,d }) => (
                  <div key={n} style={{ padding:"20px",border:`1px solid ${D.rule}`,background:D.bg1,transition:"all .2s",cursor:"default" }}
              ...
              <div style={{ border:`1px solid ${D.rule}`,overflow:"hidden" }}>
                ...
                {[
                  ...
                  <div key={i} style={{ display:"grid",gridTemplateColumns:"28px 1fr auto",gap:12,padding:"11px 16px",borderBottom:i<6?`1px solid ${D.rule}`:"none",background:D.bg2,alignItems:"center",transition:"background .12s",cursor:"default" }}
      ...
      {/* ── §03 FIVE STAKEHOLDERS — dark header + image, then bento ── */}
      <section id="roles" style={{ borderTop:`1px solid ${D.rule}` }}>

        {/* Header slide: text left | roles image right */}
        <div className="slide-grid" style={{ minHeight:360 }}>
          <div style={{ padding:"72px 7vw 72px 8vw", background:D.bgDark, display:"flex", flexDirection:"column", justifyContent:"center" }}>
            <Fade>
              ...
              <h2 style={{ fontFamily:D.display,fontSize:"clamp(26px,3.2vw,52px)",fontWeight:700,lineHeight:1.02,letterSpacing:"-.028em",marginBottom:24 }}>
              ...
              <p style={{ fontFamily:D.sans,fontSize:"16px",color:"rgba(232,238,246,0.50)",lineHeight:1.75,maxWidth:440 }}>
          ...
          <Fade delay={80} style={{ display:"contents" }}>
            <div style={{ position:"relative", background:D.bgDark, overflow:"hidden", minHeight:360 }}>
              ...
              <div style={{ position:"absolute",inset:0,background:"linear-gradient(to right,rgba(10,15,28,0.65) 0%,transparent 35%,transparent 65%,rgba(10,15,28,0.40) 100%),linear-gradient(to bottom,rgba(10,15,28,0.30) 0%,transparent 20%,transparent 80%,rgba(10,15,28,0.30) 100%)" }}/>
              {/* Role pills */}
              <div style={{ position:"absolute",top:"50%",right:48,transform:"translateY(-50%)",display:"flex",flexDirection:"column",gap:8 }}>
                {[
                  ...
                  { label:"AI Agents",              color:D.indigo },
                ].map(({ label,color }) => (
                  <div key={label} style={{ display:"flex",alignItems:"center",gap:8,padding:"6px 14px",background:"rgba(10,15,28,0.72)",border:`1px solid ${color}33`,backdropFilter:"blur(8px)" }}>
        ...
        {/* Bento grid */}
        <div style={{ background:D.bg1, padding:"4px 0 0 0" }}>
          {/* Row 1 */}
      ...
      {/* ── §04 MCP — slide deck: text left | terminal right ── */}
      ...
      {/* ── §05 AUDIT — slide deck: text + stats left | audit image right, feed below ── */}
      <section id="audit" style={{ borderTop:`1px solid ${D.rule}` }}>
        <div className="slide-grid" style={{ minHeight:520 }}>

          {/* TEXT COLUMN */}
          <div style={{ padding:"88px 7vw 72px 8vw", background:D.bg1, display:"flex", flexDirection:"column", justifyContent:"center" }}>
            <Fade>
              <div style={{ fontFamily:D.mono,fontSize:"11px",letterSpacing:".14em",color:D.ink30,textTransform:"uppercase",marginBottom:40 }}>05 — The Audit is Public</div>
              <h2 style={{ fontFamily:D.display,fontSize:"clamp(26px,3.2vw,48px)",fontWeight:700,lineHeight:1.03,letterSpacing:"-.028em",color:D.ink,marginBottom:24 }}>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:3 }}>
                {[
                  ...
                  <div key={label} style={{ display:"flex",justifyContent:"space-between",padding:"14px 18px",border:`1px solid ${D.rule}`,background:D.bg2 }}>
          ...
          {/* IMAGE COLUMN — pie chart: aggregated audit data */}
          <Fade delay={80} style={{ display:"contents" }}>
            <div style={{ position:"relative", background:D.bgDark, overflow:"hidden", minHeight:480 }}>
              ...
              <div style={{ position:"absolute",inset:0,background:"linear-gradient(to right,rgba(10,15,28,0.50) 0%,transparent 30%,transparent 70%,rgba(10,15,28,0.40) 100%),linear-gradient(to bottom,rgba(10,15,28,0.35) 0%,transparent 20%,transparent 80%,rgba(10,15,28,0.35) 100%)" }}/>
              {/* Live stats overlay */}
              <div style={{ position:"absolute",top:48,right:48 }}>
                <div style={{ padding:"20px 24px",background:"rgba(10,15,28,0.80)",border:"1px solid rgba(232,238,246,0.08)",backdropFilter:"blur(12px)" }}>
                  <div style={{ fontFamily:D.mono,fontSize:"10px",letterSpacing:".10em",color:"rgba(232,238,246,0.35)",marginBottom:14,textTransform:"uppercase" }}>Live · Devnet</div>
                  {[
                    { label:"Approved",  value:"87%",  color:D.teal   },
                    { label:"Denied",    value:"13%",  color:D.red    },
                    { label:"Entries",   value:"∞",    color:D.indigo },
                  ].map(({ label,value,color }) => (
                    <div key={label} style={{ display:"flex",justifyContent:"space-between",gap:24,marginBottom:8 }}>
              ...
              <div style={{ position:"absolute",bottom:36,left:36 }}>
                <div style={{ fontFamily:D.mono,fontSize:"10px",letterSpacing:".10em",color:"rgba(232,238,246,0.30)",textTransform:"uppercase" }}>Immutable · On-Chain · FATF R.16</div>
              </div>
            </div>
          </Fade>
        </div>
      ...
      {/* ── CTA — dark section with hero-style blended images ── */}
      <section style={{ position:"relative",overflow:"hidden",background:D.bgDark }}>
        {/* img-cta.png as full-bleed left-side blend */}
        <div style={{ position:"absolute",inset:0,backgroundImage:"url('/img-cta.png')",backgroundSize:"contain",backgroundRepeat:"no-repeat",backgroundPosition:"left center",opacity:0.28,pointerEvents:"none" }}/>
        {/* img-identity.png as full-bleed right-side blend */}
        <div style={{ position:"absolute",inset:0,backgroundImage:"url('/img-identity.png')",backgroundSize:"contain",backgroundRepeat:"no-repeat",backgroundPosition:"right center",opacity:0.25,pointerEvents:"none" }}/>
        {/* Indigo radial glow center */}
        <div style={{ position:"absolute",inset:0,background:"radial-gradient(ellipse at center,rgba(27,79,216,0.32) 0%,rgba(10,15,28,0.70) 55%,rgba(10,15,28,0.92) 100%)",pointerEvents:"none" }}/>
        {/* Grid texture */}
        <div style={{ position:"absolute",inset:0,backgroundImage:"linear-gradient(rgba(255,255,255,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.03) 1px,transparent 1px)",backgroundSize:"60px 60px",pointerEvents:"none" }}/>

        <div style={{ position:"relative",zIndex:1,padding:"140px 8vw" }}>
          <Fade>
            <div style={{ textAlign:"center",maxWidth:780,margin:"0 auto" }}>
              <div style={{ fontFamily:D.mono,fontSize:"11px",letterSpacing:".14em",color:"rgba(232,238,246,0.35)",textTransform:"uppercase",marginBottom:28 }}>Get Started</div>
              ...
              <p style={{ fontFamily:D.sans,fontSize:"18px",color:"rgba(232,238,246,0.55)",lineHeight:1.75,marginBottom:52,maxWidth:440,margin:"0 auto 52px" }}>
